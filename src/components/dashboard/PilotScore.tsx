...
export default function PilotScore({ score }: Props) {
  const radius = 54;
  const circumference = 2 * Math.PI * radius;
  const dashOffset = circumference - (score / 100) * circumference;

  const color =
    score >= 80 ? "#22c55e" : score >= 60 ? "#fbc024" : "#ef4444";

  const label =
    score >= 80 ? "Mükemmel" : score >= 60 ? "Dikkat" : "Kritik";
  ...
  return (
    <div className="glass-card rounded-2xl p-6 flex flex-col items-center gap-4">
      ...
      <div className="w-full space-y-2 mt-2">
        ...
        {[
          { label: "Acil Durum Fonu", score: 35 },
          { label: "Nakit Akışı", score: 52 },
          { label: "Stok Yönetimi", score: 61 },
        ].map(({ label, score: s }) => (
          <div key={label} className="flex items-center gap-2">
            <span className="text-xs text-[#94a3b8] w-32 truncate">{label}</span>
            <div className="flex-1 h-1.5 rounded-full bg-[rgba(255,255,255,0.05)]">
              <div
                className="h-full rounded-full"
                style={{
                  width: `${s}%`,
                  backgroundColor: s >= 80 ? "#22c55e" : s >= 60 ? "#fbc024" : "#ef4444",
                }}
              />
            </div>
